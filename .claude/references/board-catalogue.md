...
| Raspberry Pi 5 | `raspberrypi5` | `kas/machines/raspberrypi5.yml` | not wired (image builds; not yet booted) | Second board. Mainline 6.18 kernel (ADR-0011), U-Boot 2026.01, GPT layout, board facts in `conf/machine/include/edge-board-raspberrypi5.inc`. DEEPX DX-M1 composed via `kas/accel/dxm1.yml`; `edge-image-dev` with the DX stack builds and passes artifact inspection. Boot pending. |
