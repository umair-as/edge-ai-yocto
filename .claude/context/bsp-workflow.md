...
4. **Vendor BSP is machine-selected, not global.** For RZ/V2L, machine configs, kernel (`linux-renesas` → `rz_linux-cip`), U-Boot and TF-A come from `meta-renesas`, composed by `kas/bsp/renesas-rz.yml` from `kas/machines/rzv2l.yml`. Extend/override from `meta-edge-bsp/dynamic-layers/meta-rz-{bsp,distro}/`; never fork the vendor layer. A second board adds its own `kas/bsp/<vendor>.yml` and its own dynamic-layers subtree — `kas/base.yml` and `meta-edge-bsp/conf/layer.conf` are not edited.
