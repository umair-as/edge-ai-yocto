...
DESCRIPTION = "Ships /etc/sysctl.d/10-edge-hardening.conf with a baseline \
...
SRC_URI = "file://10-edge-hardening.conf"
...
do_install() {
    ...
    install -m 0644 ${UNPACKDIR}/10-edge-hardening.conf \
        ${D}${sysconfdir}/sysctl.d/10-edge-hardening.conf
}

FILES:${PN} = "${sysconfdir}/sysctl.d/10-edge-hardening.conf"
